...
from benchmark_segmentation_v2 import (
    ...
    get_ap_scores,
    batch_pix_accuracy,
    batch_intersection_union as batch_intersection_union_v2,
)
...
class LeGradBaselineEvaluator:
            ...
            try:
                ...
                def compute_metrics(text_emb_1x, prompt_text=None, method_name='legrad'):
                    ...
                    if self.use_daam:
                        ...
                        heatmap = compute_lrp_heatmap(self.model, img_t, text_emb_1x)
                        method_name = 'lrp'
    ...
    def evaluate_daam_omp(
        ...
        """
        Evaluate DAAM with Key-Space OMP using specific hyperparameters.
        
        This matches the evaluate_sparse_config method from optimize_anti_hallucination.py
        but is specifically for DAAM with the given OMP parameters.
        
        Args:
            wn_use_synonyms: Include WordNet synonyms in competing concepts
            wn_use_hypernyms: Include WordNet hypernyms in competing concepts
            wn_use_hyponyms: Include WordNet hyponyms in competing concepts
            wn_use_siblings: Include WordNet siblings in competing concepts
            dict_include_prompts: Include other class prompts as competing concepts
            sparse_threshold: Threshold for binary segmentation
            atoms: Maximum number of competing concepts (limits dictionary size)
            max_dict_cos_sim: Maximum cosine similarity for dictionary filtering
            omp_beta: OMP orthogonalization strength (0.0-2.0)
            show_progress: Show progress bar
            
        Returns:
            Dict with 'correct' and 'wrong' metrics (miou, acc, map, auroc, statistics)
        """
        if not self.use_daam:
            raise ValueError("DAAM must be enabled (use_daam=True) to use evaluate_daam_omp")
        
        if wordnet_neighbors_configured is None:
            raise ImportError("sparse_encoding module not available. Cannot use WordNet neighbors.")
        ...
        iterator = range(self.limit)
        if show_progress:
            iterator = tqdm(iterator, desc="Evaluating DAAM OMP")
        ...
        for idx in iterator:
            ...
            try:
                ...
                def compute_daam_omp_metrics(target_class_name, competing_class_names=None):
                    """Compute DAAM OMP heatmap and metrics."""
                    ...
                    # Limit by atoms parameter (like limiting dictionary size)
                    ...
                    if unique_competing:
                        ...
                        # Fallback to basic DAAM without OMP
                        heatmap = self.daam_segmenter.predict(base_img, prompt_text, size=512)
...
def main():
    ...
    # DAAM OMP settings (Key-Space OMP parameters)
    parser.add_argument('--use_daam_omp', action='store_true',
                        help='Use DAAM with Key-Space OMP (requires --use_daam)')
    parser.add_argument('--wn_use_synonyms', action='store_true',
                        help='Include WordNet synonyms in competing concepts')
    parser.add_argument('--wn_use_hypernyms', action='store_true',
                        help='Include WordNet hypernyms in competing concepts')
    parser.add_argument('--wn_use_hyponyms', action='store_true',
                        help='Include WordNet hyponyms in competing concepts')
    parser.add_argument('--wn_use_siblings', action='store_true',
                        help='Include WordNet siblings in competing concepts')
    parser.add_argument('--dict_include_prompts', action='store_true',
                        help='Include other class prompts as competing concepts')
    ...
    parser.add_argument('--atoms', type=int, default=16,
                        help='Maximum number of competing concepts (default: 16)')
    ...
    # Run evaluation
    if args.use_daam and args.use_daam_omp:
    ...
    # Choose evaluation method based on flags
    if args.use_daam_omp:
        if not args.use_daam:
            print("[WARNING] --use_daam_omp requires --use_daam. Enabling DAAM mode.")
            args.use_daam = True
        
        results = evaluator.evaluate_daam_omp(
