...
class LeWrapper(nn.Module):
    ...
    def compute_gradcam_vit(self, text_embedding, image=None, target_layer: int = 8):
        """
        GradCAM-style explanation for ViT-based OpenCLIP models.

        This implementation follows the GradCAM formulation adapted for ViTs
        described in the LeGrad paper by treating the intermediate tokens as
        activations. Concretely, for a chosen layer l with tokens Z^l the weights
        are the gradients of a similarity score s w.r.t. token dimensions,
        averaged over all tokens; the final map is the (ReLU-ed) weighted
        average of token embeddings across channels, reshaped to the patch grid.

        Args:
            text_embedding: Tensor of shape [num_prompts, D] (normalized).
            image: Tensor of shape [1, 3, H, W] or [B, 3, H, W]. If provided,
                   it will be repeated for each prompt.
            target_layer: Index of the transformer block (0-based) to use for GradCAM.
                          The LeGrad paper reports layer 8 of ViT-B/16 works well.

        Returns:
            heatmaps: Tensor of shape [1, num_prompts, H_out, W_out] with values in [0, 1].
        """
        if self.model_type != 'clip':
            raise ValueError("compute_gradcam_vit is currently implemented only for OpenCLIP ViT models ('clip').")

        num_prompts = text_embedding.shape[0]

        # --- Forward image through the vision tower to populate hooks ---
        if image is not None:
            # Repeat image for each prompt so we can compute a per-prompt score.
            if image.shape[0] == 1 and num_prompts > 1:
                image = image.repeat(num_prompts, 1, 1, 1)
            H_img, W_img = image.shape[-2], image.shape[-1]
            _ = self.encode_image(image)
        else:
            # Still run a dummy forward if tokens are not yet populated.
            raise ValueError("compute_gradcam_vit currently expects an input image tensor.")

        # --- Select the requested transformer block ---
        blocks_list = list(dict(self.visual.transformer.resblocks.named_children()).values())
        if target_layer < 0:
            layer_idx = len(blocks_list) + target_layer
        else:
            layer_idx = target_layer
        if layer_idx < 0 or layer_idx >= len(blocks_list):
            raise ValueError(f"target_layer={target_layer} is out of range for {len(blocks_list)} blocks.")
        ...
        L, B, D = Z.shape
        if B != num_prompts:
            # In case image batch does not match prompts (should not happen in normal usage)
            raise RuntimeError(f"Batch size mismatch between tokens (B={B}) and text embeddings (num_prompts={num_prompts}).")

        # --- Build an image embedding from this layer's tokens (like CLIP's head) ---
        # Average over tokens, then apply the same projection head as CLIP.
        img_feat_l = self.visual.ln_post(Z.mean(dim=0)) @ self.visual.proj  # [B, D_out]
        img_feat_l = F.normalize(img_feat_l, dim=-1)

        # Similarity score s between text and image features at this layer.
        # text_embedding: [P, D_out], img_feat_l: [B, D_out] with B == P.
        sim = text_embedding @ img_feat_l.transpose(-1, -2)  # [P, B]
        # Use the sum of diagonal elements (per-prompt similarity) as scalar score.
        diag = torch.diagonal(sim, 0)  # [P]
        score = diag.sum()

        # --- Gradients of score w.r.t. token embeddings Z^l ---
        self.visual.zero_grad(set_to_none=True)
        grad = torch.autograd.grad(score, Z, retain_graph=False, create_graph=False)[0]  # [L, B, D]

        # GradCAM weights: average gradients over all tokens (including CLS).
        # w_bk = (1 / L) * sum_i d s / d z_i^l[k]
        w = grad.mean(dim=0)  # [B, D]

        # Token importance: weighted average of token embeddings over channels, ReLU-ed.
        # We discard the CLS token (index 0) when forming the spatial map.
        tokens_no_cls = Z[1:, :, :]  # [N, B, D] where N = L - 1
        # Broadcast weights over tokens: [N, B, D]
        weighted_tokens = tokens_no_cls * w.unsqueeze(0)
        # Average over channel dimension and apply ReLU: [N, B]
        cam = weighted_tokens.mean(dim=-1)  # [N, B]
        cam = torch.clamp(cam, min=0.0)

        # Reshape to spatial grid (patch grid).
        num_tokens = tokens_no_cls.shape[0]
        if hasattr(self.visual, 'last_grid_size'):
            h, w_grid = self.visual.last_grid_size
        else:
            side = int(math.sqrt(num_tokens))
            h, w_grid = side, side
        if h * w_grid != num_tokens:
            # Fallback to a square grid if needed.
            side = int(math.sqrt(num_tokens))
            h = w_grid = side

        cam = cam.permute(1, 0)  # [N, B] -> [B, N]
        expl_map = cam.view(B, 1, h, w_grid)  # [B, 1, h, w]

        # Upsample to image size and min-max normalize.
        expl_map = F.interpolate(expl_map, size=(H_img, W_img), mode='bilinear', align_corners=False)  # [B,1,H,W]
        # Re-arrange to [1, num_prompts, H, W] to match LeGrad's output convention.
        expl_map = expl_map.permute(1, 0, 2, 3)  # [1, B, H, W]
        expl_map = min_max(expl_map)  # [1, B, H, W]
        return expl_map
