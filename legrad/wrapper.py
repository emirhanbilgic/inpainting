...
class LeWrapper(nn.Module):
    ...
    def compute_gradcam_clip(self, text_embedding, image=None, target_layer=None, allow_fallback=False):
        ...
        def _forward_hook(module, inp, out):
            ...
            # store patch tokens for the spatial CAM reconstruction only
            activations["patches"] = tokens[1:].permute(1, 0, 2)  # [B, L_p, C]

            # capture gradients w.r.t. all tokens (incl. CLS) to follow the paper's definition
            def _tensor_grad_hook(grad):
                gradients["tokens"] = grad.permute(1, 0, 2)  # [B, L_full, C]

            tokens.register_hook(_tensor_grad_hook)
        ...
        try:
            ...
            # Backward pass: populates gradients["tokens"]
            self.zero_grad()
            score.backward(retain_graph=False)

            if "patches" not in activations or "tokens" not in gradients:
                raise RuntimeError("Grad-CAM hooks did not capture activations/gradients.")

            feats_patches = activations["patches"]   # [P, L_p, C]
            grads_tokens = gradients["tokens"]       # [P, L_full, C]

            # Average gradients across all tokens (CLS + patches) as described in the paper
            weights = grads_tokens.mean(dim=1)      # [P, C]
        ...
        if degenerate and allow_fallback:
            ...
            def make_fwd_hook(layer_idx):
                def _fwd(module, inp, out):
                    ...
                    # store patch tokens only for reconstruction
            ...
            for li in range(self.starting_depth, len(blocks_list)):
                ...
                G_full = grads_by_layer.get(li, None)  # [P, L_full, C]
                if A is None or G_full is None:
                    continue
                wts = G_full.mean(dim=1)           # [P, C]
